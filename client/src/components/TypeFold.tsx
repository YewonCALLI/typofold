    ...
    <div className='relative flex flex-row w-screen h-screen'>
      ...
      <div className='flex-1 relative'>
        ...
        <div className='absolute left-5 bottom-5 text-black p-[10px] z-[100]'>
          <div className='text-[16px] text-center font-extralight pb-[15px]'>Choose an alphabet!</div>
          <div className='flex w-[200px] flex-wrap justify-center items-center gap-[10px]'>
            {alphabets.map((alphabet) => (
              <button
                key={alphabet.type}
                style={fileURL === alphabet.path ? { backgroundColor: '#000', color: '#fff' } : {}}
        ...
        <ControlPanel
          ...
          <button
            id='captureButton'
            className='flex justify-center items-center w-[110px] h-fit px-4 py-2 bg-white cursor-pointer text-[16px] font-light rounded-[30px] border border-[#ff00f2] text-[#ff00f2] hover:bg-[#ff00f2] hover:text-white transition-colors'>
            Print 🖨️
          </button>
        </ControlPanel>
        ...
        <Canvas
          ...
          <spotLight position={[10, 10, 10]} angle={0.15} penumbra={1} intensity={7.6} color='#fff9f0' />
      ...
      <div
        ...
        <UnfoldedFace onTextureReady={handleTextureReady} faceSize={faceGroups ? faceGroups[0].size : null} />
      </div>
    </div>
  )
}
